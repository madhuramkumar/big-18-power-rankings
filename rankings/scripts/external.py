...
def get_team_records() -> Dict[str, str]:
    ...
    for team_name, team_id in name_to_alias_mapping().items():
        ...
        kenpom_rank = kenpom_ranks.get(team_id, "N/A")
        team_records[team_id] = f"{record}, TRank: {torvik_rank}, KP: {kenpom_rank}"
...
def get_kenpom() -> Dict[str, int]:
    ...
    kenpom_ranks: Dict[str, int] = {}
    
    for index, row in df.iterrows():
        team_name = row['TeamName'].replace("St.", "State")
        if team_name in name_to_id:
            team_id = name_to_id[team_name]
            rank = row['RankAdjEM']
            if rank is not None:
                kenpom_ranks[team_id] = int(rank)
